...
// Ensure users table includes role column if schema has been upgraded
$sql_check_role = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
                      WHERE TABLE_NAME='users' AND COLUMN_NAME='role' AND TABLE_SCHEMA='test'";
$result_role = $conn->query($sql_check_role);

if ($result_role && $result_role->num_rows == 0) {
    $sql_alter_users = "ALTER TABLE test.users ADD COLUMN role ENUM('user','admin','moderator') NOT NULL DEFAULT 'user' AFTER password_hash";
    if ($conn->query($sql_alter_users) === TRUE) {
        $messages[] = "✓ Role column added to users table";
    } else {
        $messages[] = "✗ Error modifying users table: " . $conn->error;
    }
} else {
    $messages[] = "✓ Users table already has role column";
}
...
$sql_archive = "CREATE TABLE IF NOT EXISTS test.archive_tasks (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
if ($conn->query($sql_archive) === TRUE) {
    $messages[] = "✓ Archive tasks table created successfully";
} else {
    $messages[] = "✗ Error creating archive table: " . $conn->error;
}

// Create deleted_tasks table for permanent deletion audit logs
$sql_deleted = "CREATE TABLE IF NOT EXISTS test.deleted_tasks (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    title VARCHAR(255) NOT NULL,
    description TEXT,
...
if ($conn->query($sql_deleted) === TRUE) {
    $messages[] = "✓ Deleted tasks table created successfully";
} else {
    $messages[] = "✗ Error creating deleted tasks table: " . $conn->error;
}

// Create task_stats table for analytics
$sql_stats = "CREATE TABLE IF NOT EXISTS test.task_stats (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
if ($conn->query($sql_stats) === TRUE) {
    $messages[] = "✓ Task stats table created successfully";
} else {
    $messages[] = "✗ Error creating stats table: " . $conn->error;
}
    ...
    <div class="container">
        ...
        <?php if ($setup_complete): ?>
            <div class="success-info">
                ...
                <p>Your database has been configured with users, tasks, archive, and analytics tables.</p>
                <p><strong>Features Enabled:</strong></p>
                <ul>
                    <li>User registration & authentication</li>
                    <li>Task management (add, edit, complete, archive)</li>
                    <li>Archive & restore deleted tasks</li>
                    <li>Task analytics & insights</li>
                </ul>
            </div>
        <?php endif; ?>
